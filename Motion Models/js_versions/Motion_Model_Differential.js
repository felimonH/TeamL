function forwardKinematics (R, omega, ICC, x, y, theta, t) {
...
function straightMotion (v, x, y, theta, t) {
    let x_new = x + (v * t * np.cos(theta));
    let y_new = y + (v * t * np.sin(theta));
    return [x_new, y_new, theta];
}

function robotStep (r_l, r_r, L, u_l, u_r, x, y, theta, t) {
    let v_l = u_l * r_l;
    let v_r = u_r * r_r;
    let result = [];
...
function main (r_l, r_r, L, u_l, u_r, x, y, theta) {
    let robot_path = [];
